...
const RevenueCountryLiquor = ({ year }) => {
    // api call    
    const [revenuecl, setrevenuecl] = useState([]);
    useEffect(() => {
        axios.get(`/api/v1/revenue/countryLiquor?year=${year}`) // Use the proxied URL
            .then((res) => {
                setrevenuecl(res.data);
    ...
    // console.log("revernue cl", year)
    // console.log(revenuecl)

    // revenuecl.forEach(data => {
    //     console.log(data)
    // });
...
export default RevenueCountryLiquor
