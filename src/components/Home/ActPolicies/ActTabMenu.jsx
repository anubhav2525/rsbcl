...
import ActExciseTab from './Excise/ActExciseTab'
import ActrsbclTab from './RSBCL/ActrsbclTab'
import ActrsgsmTab from './RSGSM/ActrsgsmTab'

const ActTabMenu = () => {

    const [activeacts, setactiveacts] = useState(<ActExciseTab />);
    const [activeTab, setactiveTab] = useState("excise");

    useEffect(() => {
        console.log(activeTab)
        console.log(activeacts)
    },[activeTab])
            ...
            <div className='w-full flex items-center justify-between my-3'>
                <div className="w-full">
                    <ul className="w-full flex justify-between text-sm font-bold">
                        <li>
                            <button type="button"className={`inline-block  px-2 py-2 border-y-2 rounded-md  ${activeTab === "excise" ? "text-blue-600 border-blue-600 dark:text-blue-700 dark:border-blue-600" : "text-slate-800 dark:text-slate-300 border-slate-500 hover:text-blue-600 hover:border-blue-500 dark:hover:text-blue-600 dark:hover:border-blue-600"}`} onClick={() => {
                                setactiveacts(<ActExciseTab />)
                                setactiveTab("excise")
                            }}>
                                Excise
                            </button>
