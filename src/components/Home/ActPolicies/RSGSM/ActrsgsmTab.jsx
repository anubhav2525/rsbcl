...
const ActrsgsmTab = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`/api/v1/public/acts-policies/latest-five/rsgsm`);
    ...
    const handleNavigate = () => {
        const data = {
            api: '/api/v1/public/acts-policies',
            title: 'Acts',
            department: 'RSGSM',
            heading: 'Acts details'
        };
        navigate('/acts', { state: data });
    };
    ...
    return (
        <div className="w-full">
            {
                data.length == 0 && <div className='w-full h-full border-t py-8 font-bold flex justify-center items-center'>No Content</div>
            }
            {data.map((item, index) => {
            ...
            <div className='pt-4'>
                <button
                    ...
                    className="bg-gradient-to-r from-blue-800 to-indigo-900 text-white font-medium rounded-full text-sm px-5 py-2 text-center hover:bg-gradient-to-r hover:from-blue-600 hover:to-violet-600">View all</button>
            </div>
        </div>
    )
}

export default ActrsgsmTab
