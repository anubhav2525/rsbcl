...
const RevenueImfl = ({ year }) => {
    // api call    
    const [revenueiml, setrevenueiml] = useState([]);
    useEffect(() => {
        axios.get(`/api/v1/revenue/iml?year=${year}`) // Use the proxied URL
            .then((res) => {
                setrevenueiml(res.data);
    ...
    // console.log("revernue iml", year)
    // console.log(revenueiml)

    // revenueiml.forEach(data => {
    //     console.log(data)
    // });
