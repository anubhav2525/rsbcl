...
const MobileAppDelete = ({ setdeleteModal, deleteModal, id, setAlertMsg, fetchData }) => {
    const [removedStatus, setremovedStatus] = useState(false);
    const [msg, setMsg] = useState("");

    const handleDeleteRecordById = async () => {
        try {
            const response = await axios.delete(`/api/v1/auth/mobile-app/id/${id}`);
    ...
    return (
        <div>
            <div>
                ...
                <h3 className="mb-2 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this app?</h3>
                <p className="mb-5 text-sm font-normal text-gray-500 dark:text-gray-400">Application id: {id}</p>
