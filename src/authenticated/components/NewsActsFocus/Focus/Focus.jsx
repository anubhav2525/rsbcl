...
import FocusDelete from "../Focus/FocusDelete"
...
const Focus = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('/api/v1/public/inFocuses-page', {
        params: {
      ...
      if (response.status === 200) {
        setacts(response.data.data.content);
    ...
    <div className='h-full w-full p-4 md:px-2 '>
      ...
      <div className='mb-3 w-full grid grid-cols-1 md:grid-cols-3 gap-2'>
        ...
        <div className='flex justify-end items-center gap-x-1 col-span-2'>
          {/* add button  */}
          <div>
            <Link to="/authenticated/focus/add" className="bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 text-white rounded shadow-md hover:opacity-90 py-2 px-3 h-full text-xs flex justify-between items-center gap-x-2">
              <div>
          ...
          <table className="w-full rounded-lg table-auto text-sm text-left">
            <thead className="dark:bg-blue-700 bg-blue-700 text-gray-100 font-medium border-b">
              <tr>
                ...
                <th className="py-2 px-6">Link</th>
                ...
                </tr>}
              {
                acts.map((item, index) =>
                  <tr key={index}>
                    ...
                    <td className="px-6 py-3 capitalize">{item.link}</td>                    
                    ...
                    <td className="flex py-2 justify-end items-center">
                      <Link to={`/authenticated/focus/update/id/${item.id}`} className="py-2 px-3 font-medium text-indigo-600 hover:text-indigo-500 duration-150 hover:bg-gray-100 rounded-md">
