...
import ServiceProvidersAuthenticatedRemove from "./ServiceProvidersAuthenticatedRemove"

const ServiceProvidersAuthenticated = () => {
    // alert message
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('/api/v1/public/services', {
                params: {
        ...
        <div className='h-full w-full p-4 md:px-2 '>
            ...
            <div className='mb-3 w-full grid grid-cols-1 md:grid-cols-3 gap-2'>
                ...
                <div className='flex justify-end items-center gap-x-1 col-span-2'>
                    {/* add button  */}
                    <div>
                        <Link to="/authenticated/services/add" className="bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 text-white rounded shadow-md hover:opacity-90 py-2 px-3 h-full text-xs flex justify-between items-center gap-x-2">
                            <div>
            ...
            <div>
                    ...
                    <table className="w-full rounded-lg table-auto text-sm text-left">
                        <thead className="dark:bg-blue-700 bg-blue-700 text-gray-100 font-medium border-b">
                            <tr>
                                ...
                                <th colSpan={2} className="py-2 px-6">Link</th>
...

                            {
                                services.map((item, index) =>
                                    <tr key={index}>
                                        ...
                                        <td className="px-6 py-3 whitespace-nowrap text-blue-700 font-bold hover:text-blue-500 hover:underline" colSpan={2}>
                                            <a href={item.link} target="_blank">{item.link}</a>
                                        </td>
                                        <td className='px-6 py-3 capitalize'>
                                            {/* ACTIVE|REMOVED|DELETED|INACTIVE */}
                                            {item.status === 'ACTIVE' && <span className="bg-green-600 text-white text-xs font-medium me-2 px-3 py-0.5 rounded dark:bg-green-700 dark:text-white">
                                                {item.status}
                                            </span>}
                                            {item.status === 'REMOVED' && <span className="bg-yellow-600 text-white text-xs font-medium me-2 px-3 py-0.5 rounded dark:bg-yellow-700 dark:text-white">
                                                {item.status}
                                            </span>}
                                            {item.status === 'DELETED' && <span className="bg-red-600 text-white text-xs font-medium me-2 px-3 py-0.5 rounded dark:bg-red-700 dark:text-white">
                                                {item.status}
                                            </span>}
                                            {item.status === 'INACTIVE' && <span className="bg-sky-600 text-white text-xs font-medium me-2 px-3 py-0.5 rounded dark:bg-sky-700 dark:text-white">
                                                {item.status}
                                            </span>}
                                        </td>
                                        ...
                                        <td className="flex py-2 justify-end items-center">
                                            <Link to={`/authenticated/services/edit/id/${item.id}`} className="py-2 px-3 font-medium text-indigo-600 hover:text-indigo-500 duration-150 hover:bg-gray-100 rounded-md">
             ...
             {deleteModal && (
                <ServiceProvidersAuthenticatedRemove
                    fetchData={fetchData}
...
export default ServiceProvidersAuthenticated
